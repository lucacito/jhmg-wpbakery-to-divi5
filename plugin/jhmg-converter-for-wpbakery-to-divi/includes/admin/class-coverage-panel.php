...
class CoveragePanel {
    ...
    public function markup(): string {
        ...
        if ( empty( $coverage ) ) {
            return '<div class="wbdc-card wbdc-card--success"><h2>' . esc_html__( 'Conversion coverage', 'jhmg-converter-for-wpbakery-to-divi' ) . '</h2><p>'
                . esc_html__( 'Everything converted. No WPBakery elements were left unsupported across your recent conversions.', 'jhmg-converter-for-wpbakery-to-divi' )
        ...
        return sprintf(
            ...
            esc_html__( 'Conversion coverage', 'jhmg-converter-for-wpbakery-to-divi' ),
            esc_html__( 'WPBakery elements from your recent conversions that have no Divi 5 equivalent yet. These need rebuilding by hand. A theme or add-on element is not listed here: it is kept, as static HTML or as a labelled placeholder, and named in each page\'s own report.', 'jhmg-converter-for-wpbakery-to-divi' ),
            esc_html__( 'Element', 'jhmg-converter-for-wpbakery-to-divi' ),
            esc_html__( 'Runs affected', 'jhmg-converter-for-wpbakery-to-divi' ),
            esc_html__( 'Last seen', 'jhmg-converter-for-wpbakery-to-divi' ),
            $rows,
            $this->consent_line()
        ) . $this->runs_table();
    }
    ...
    private function consent_line(): string {
        ...
        return sprintf(
            ...
            $on
                ? esc_html__( 'Sharing this list of element names with divi5lab so these gaps get prioritised. No site address, no content, no personal data.', 'jhmg-converter-for-wpbakery-to-divi' )
                : esc_html__( 'Help prioritise these elements? Sharing sends only the element names above and the theme families they came from, once a week. No site address, no content, no personal data.', 'jhmg-converter-for-wpbakery-to-divi' ),
            esc_url( $url ),
            $on
                ? esc_html__( 'Stop sharing', 'jhmg-converter-for-wpbakery-to-divi' )
                : esc_html__( 'Share these element names', 'jhmg-converter-for-wpbakery-to-divi' )
        );
    }
        ...
        foreach ( $this->history->all() as $run ) {
            ...
            if ( ! empty( $run['rolled_back'] ) ) {
                ...
                $undo = sprintf(
                    ...
                    esc_html__( 'Undo', 'jhmg-converter-for-wpbakery-to-divi' )
                );
            }
