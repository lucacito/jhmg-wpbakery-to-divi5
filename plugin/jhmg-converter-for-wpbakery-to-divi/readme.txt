...
Counted by `scripts/element-coverage.php` over WPBakery's own 75 bundled templates, a commercial 35-layout pack and 96 real theme exports: **75 of the 75 elements WPBakery registers** reach a handler written for them (38 exact, 31 approximate, 6 read by their parent element), plus **23 template-only and vendor tags** and **30 of the 73 theme and add-on elements** those corpora hold.
...
**Theme and add-on elements** — a WPBakery site usually runs a ThemeForest theme whose own elements outnumber WPBakery's on the page. None of them is ever dropped. Converting a page on this site renders it with the theme active and keeps the result as static HTML, exactly as the page looks today; converting from an export leaves a labelled placeholder holding the shortcode and its text. Either way the report names the family and the count — "Ronneby × 12" — rather than a list of tags nobody recognises. Recognised families include Ronneby, The Retailer, Ultimate Addons, Massive Addons, Salient, Bridge, The7, Jupiter, Templatera, WooCommerce, Contact Form 7 and Gravity Forms.

**Ultimate Addons for WPBakery** — the six most common elements convert into real Divi modules rather than static copies: `ult_content_box`, `bsf-info-box`, `just_icon`, `ultimate_pricing`, `stat_counter` and `ultimate_video`. Ronneby's own element set converts the same way.
