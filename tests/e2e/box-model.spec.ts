...
/**
 * Measures WPBakery's box model against two hand-written Divi 5 candidates, so
...
async function measure(page: import('@playwright/test').Page, kind: 'wpbakery' | 'divi'): Promise<Measured> {
  return page.evaluate((args) => {
    ...
    return { containerLeft, rows: out, moduleGap };
  }, { selectors: SELECTORS[kind] });
}

test.describe.serial('WPBakery box model versus two Divi 5 candidates', () => {
  ...
  for (const subject of cases) {
    test(`measures ${subject.key}`, async ({ page }) => {
      ...
      expect(measured.rows.length, 'both rows were found').toBe(2);
